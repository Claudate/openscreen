...
const Mode = (props: ModeProps) => {
	...
	const labelFor = (mode: RecordingMode) =>
		mode === "instant"
			? t("mode.instantMode")
			: mode === "studio"
				? t("mode.studioMode")
				: t("mode.screenshotMode");

	const descriptionFor = (mode: RecordingMode) =>
		mode === "instant"
			? t("mode.instantModeDesc")
			: mode === "studio"
				? t("mode.studioModeDesc")
				: t("mode.screenshotModeDesc");
	...
	return (
		<div class="flex relative gap-2 items-center p-1.5 rounded-full border border-gray-5 bg-gray-3 w-fit">
			<button
				...
				aria-label={t("mode.recordingModeInfo")}
			>
								...
								<div class="flex flex-col gap-2 px-3 py-2.5 rounded-lg border shadow-lg bg-gray-12 text-gray-1 border-gray-3 min-w-[12rem] max-w-[15rem]">
									<div class="flex flex-col gap-0.5">
										<span class="text-xs font-medium">
											{labelFor(button.mode)}
										</span>
										<span class="text-[10px] text-gray-4 leading-snug">
											{descriptionFor(button.mode)}
										</span>
									</div>
									<Show when={button.settingsSection}>
										{(section) => (
											...
											>
												...
												<span>{t("mode.qualitySettings")}</span>
											</button>
										)}
									</Show>
								</div>
