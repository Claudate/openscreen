	...
	return (
		<div class="cap-settings-page flex flex-col h-full custom-scroll">
			<SettingsPageContent>
				...
				>
					<SectionCard padded class="space-y-3">
						<div class="flex items-center justify-between gap-3">
							<div class="flex flex-col gap-0.5 min-w-0">
								<p class="text-[13px] text-gray-12">
									{t("settings.transcription.rememberedTerms")}
								</p>
								<p class="text-xs leading-snug text-gray-10">
									{t("settings.transcription.rememberedTermsDesc")}
								</p>
							</div>
						...
						<p class="text-xs leading-relaxed text-gray-10">
							{t("settings.transcription.hintsApplied")}
						</p>
					</SectionCard>
				</Section>
				...
				<Show when={hints().length > 0}>
					<Section
						title={t("settings.transcription.activeHints")}
						right={
							<span class="text-xs text-gray-10">
								{hints().length}{" "}
								{hints().length === 1
									? t("settings.transcription.item")
									: t("settings.transcription.items")}
							</span>
						}
					>
						<SectionCard padded>
